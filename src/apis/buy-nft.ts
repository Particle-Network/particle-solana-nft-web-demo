...
import {
  ...
  NFT_BUY_ESTIMATED_FEE_LAMPORTS_COST,
...
export async function buyNFT(
  ...
  const balanceNeeded =
    NFT_BUY_ESTIMATED_FEE_LAMPORTS_COST + auctionEntity.price * LAMPORTS_PER_SOL;
  ...
  if (balance < balanceNeeded) {
    return createApiStandardResponse(
      `Insufficient balance, please make sure your balance greater or equal to ${
        balanceNeeded / LAMPORTS_PER_SOL
      } SOL`
    );
  }
  ...
  const auctionManagerData = programs.metaplex.AuctionManagerV2Data.deserialize(
    auctionManagerAI.data
  );
  ...
  const responseProcessTransactions = await processTransactions(
    provider,
    responseNFTBuy.result.transactions
  );
